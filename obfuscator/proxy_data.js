...
const DEFAULT_HOSTNAME = 'bestip.web3.dpdns.org';
// ================================================
...
async function readLocalBackup(type, name) {
    try {
        ...
        if (type === 'country') {
            filePath = path.join(dataDir, 'countries', `${name.toUpperCase()}.json`);
        } else if (type === 'region') {
            filePath = path.join(dataDir, 'regions', `${name}.json`);
        } else if (type === 'global') {
            filePath = path.join(dataDir, 'bestip.json');
        } else if (type === 'stats') {
...
async function fetchCountryBestIP(countryCode) {
    try {
        ...
        console.error(`在线获取国家 ${countryCode} 的最佳IP失败:`, error.message);
    }
    const localData = await readLocalBackup('country', countryCode);
    if (localData) {
        console.log(`成功从本地加载国家 ${countryCode} 的备份数据`);
...
async function fetchGlobalBestIP() {
    try {
        ...
        console.error('在线获取全球最佳IP失败:', error.message);
    }
    const localData = await readLocalBackup('global');
    if (localData) {
        console.log('成功从本地加载全球最佳IP备份数据');
        return Array.isArray(localData) ? localData : [];
    }
    return [];
}
...
async function fetchRegionBestIP(region, useRegex) {
    ...
    try {
        ...
        console.error(`在线获取地区 ${region} 的最佳IP失败:`, error.message);
    }
    const localData = await readLocalBackup('region', decodedRegion);
    if (localData) {
        console.log(`成功从本地加载地区 ${decodedRegion} 的备份数据`);
        return Array.isArray(localData) ? localData : [];
    }
    return [];
}
...
async function fetchStatsData() {
    try {
        ...
        console.error('在线获取统计数据失败:', error.message);
    }
    const localData = await readLocalBackup('stats');
    if (localData) {
        console.log('成功从本地加载统计数据备份');
        return localData;
    }
    return {};
}
